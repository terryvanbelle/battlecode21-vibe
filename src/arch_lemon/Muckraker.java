...
public strictfp class Muckraker extends Robot {
    ...
    private MapLocation patrol = null; private int patrolRound = -100;   // arch_hunt: current patrol point around the enemy centre
    ...
    @Override protected void turn() throws GameActionException {
        ...
        if (C.ARCHETYPE >= 6 && MapState.nEnemy > 0) {
            ...
            if (patrol == null || round - patrolRound > 15 || loc.distanceSquaredTo(patrol) <= 2) {
                ...
                if (Math.max(Math.abs(dx), Math.abs(dy)) < 3) { dx = dx < 0 ? -3 : 3; }
                patrol = e.translate(dx, dy); patrolRound = round;
            }
            nav.setTarget(patrol); if (nav.step()) return;
            patrol = null; return;
        }
