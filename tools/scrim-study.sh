...
OUT="$RUN/study.tsv"
printf 'game\topp\tmap\twon\tround\tus_ec\tus_ecInf\tus_sla\tus_muc\tus_pol\tus_exp\tus_buff\tus_unitInf\tus_cov\tus_navMoves\tus_navAba\tus_navSwamp\tth_ec\tth_ecInf\tth_sla\tth_muc\tth_pol\tth_exp\tth_buff\tth_unitInf\tth_cov\tth_navMoves\tth_navAba\tth_navSwamp\n' > "$OUT"
NAV="$RUN/nav.tsv"
...
for f in "$RUN"/losses/*.bc21 "$RUN"/replays/*.bc21; do
  ...
  nice -n 10 "$REPO/tools/replay-dump.sh" "$f" --metrics 2>/dev/null | awk -F, -v U="$U" -v T="$T" -v game="$b" -v opp="$opp" -v map="$map" -v won="$won" '
    NR==1{for(i=1;i<=NF;i++)h[$i]=i; next}
    $1%50==0 && $1>=50 && $1<=700 {printf "%s\t%s\t%s\t%s\t%s", game, opp, map, won, $1;
      for (k=1;k<=9;k++) { split("ecs ecInf sla muc pol exposes buff unitInf", c, " ") }
      printf "\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s", $h[U"_ecs"],$h[U"_ecInf"],$h[U"_sla"],$h[U"_muc"],$h[U"_pol"],$h[U"_exposes"],$h[U"_buff"],$h[U"_unitInf"],$h[U"_cov"],$h[U"_navMoves"],$h[U"_navAba"],$h[U"_navSwamp"];
      printf "\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", $h[T"_ecs"],$h[T"_ecInf"],$h[T"_sla"],$h[T"_muc"],$h[T"_pol"],$h[T"_exposes"],$h[T"_buff"],$h[T"_unitInf"],$h[T"_cov"],$h[T"_navMoves"],$h[T"_navAba"],$h[T"_navSwamp"]; }' >> "$OUT"
  echo "  studied $b" >&2
done
