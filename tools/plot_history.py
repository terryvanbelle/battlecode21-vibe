...
def plot(series, title, out, lines):
    ...
    if len(full) >= 2:
        ax.plot(full, [sum(by_date[d]) / len(by_date[d]) for d in full], "k--", lw=2.4, label="average")
    ...
    ax.legend(fontsize=7, loc="center left", bbox_to_anchor=(1.01, 0.5)); fig.tight_layout(); fig.savefig(out, dpi=140); print("wrote", out)
...
def main():
    ...
    if ladder: plot(ladder, "Ladder: win % vs external Battlecode 2021 bots (scrimmage proxy)", REPO / "progress" / "ladder.png", [(20, "20% unlock: replays may be reviewed"), (50, "50% peer")])
