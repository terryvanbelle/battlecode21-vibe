...
public strictfp class Politician extends Robot {
    ...
    private void capture() throws GameActionException {
        ...
        RobotInfo t = null;
        if (rc.canSenseLocation(target)) { t = rc.senseRobotAtLocation(target); if (t == null || t.type != RobotType.ENLIGHTENMENT_CENTER || t.team == us) { MapState.removeNeutral(target); MapState.removeEnemy(target); if (t != null && t.team == us) { MapState.addOwnEC(target); if (C.ABORT_REPORT == 1) setFlag(Comms.encode(Comms.OWN_EC, 1, target)); /* extra 1: seen, not echoed */ } role = Roles.GUARD; Debug.log("@capture abort by=" + (t == null ? "vanished" : t.type != RobotType.ENLIGHTENMENT_CENTER ? "notEC" : "ours") + " r=" + round + " age=" + (round - birth)); return; } targetInf = t.influence; }
