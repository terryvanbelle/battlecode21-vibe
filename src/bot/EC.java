...
public strictfp class EC extends Robot {
    ...
    private boolean enemyPolNear = false;   // Iteration 56b: an enemy politician in sensor range this turn
    private boolean swarm = false; private int lastSwarmRound = -1000; private int swarmGuards = 0;   // Iteration 58: a muckraker swarm seen recently
    ...
    @Override protected void turn() throws GameActionException {
        ...
        boolean econDanger = false; enemyPolNear = false; int enemyMucks = 0;
        for (int i = nEnemy; --i >= 0;) {
            RobotInfo r = enemies[i];
            if (r.type == RobotType.POLITICIAN && r.conviction >= 11) enemyPolNear = true;   // Iteration 56b: something a 15-guard cannot answer
            if (r.type == RobotType.MUCKRAKER) enemyMucks++;
        ...
        if (enemyMucks >= C.SWARM_MUCKS) lastSwarmRound = round;   // Iteration 58
        swarm = C.SWARM_MUCKS == 0 || round - lastSwarmRound <= C.SWARM_MEMORY;
    ...
    private void build(int inf, boolean danger, boolean econDanger) throws GameActionException {
        ...
        if (C.ARCHETYPE == 3) {   // politician rush: 2 scouts, 2 small slanderers, then every 100+ influence becomes a capture politician aimed at the enemy EC
        ...
        else if (danger && guards < 2 && inf >= 20) { t = RobotType.POLITICIAN; cost = (C.GUARD_MUCK_COST > 0 && !enemyPolNear && swarm) ? C.GUARD_MUCK_COST : Math.min(inf - 5, 30); role = Roles.GUARD; }
        ...
        else if (inf >= 20 && (guards < C.GUARD_BASE + slanderers / 2 || (danger && guards < C.MAX_GUARDS))) { t = RobotType.POLITICIAN; cost = (C.GUARD_MUCK_COST > 0 && !enemyPolNear && swarm) ? C.GUARD_MUCK_COST : Math.min(Math.max(20, inf / 4), inf >= C.RICH_BANK ? C.GUARD_COST_RICH : 60); role = Roles.GUARD; if (cost == C.GUARD_MUCK_COST) swarmGuards++; }   // Iteration 56b/58: cheap guards against a muckraker swarm; 59: big ones when rich
            ...
            else if (guards < (inf >= C.RICH_BANK ? C.SPEND_GUARD_CAP_RICH : C.SPEND_GUARD_CAP)) { t = RobotType.POLITICIAN; cost = Math.min(spare, Math.min(C.GUARD_COST_RICH, Math.max(20, spare / 3))); role = Roles.GUARD; }   // Iteration 59: more, bigger guards when rich, never a 3,000   // Iteration 35 dose 2: no big-bank escape -- a big bank is what buys a centre
