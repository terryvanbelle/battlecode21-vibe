...
NAV="$RUN/nav.tsv"
printf 'opp\tmap\tside\tus_cov\tth_cov\tus_moves\tth_moves\tus_meanMoves\tth_meanMoves\tus_aba\tth_aba\tus_swamp\tth_swamp\tus_firstEC\tth_firstEC\n' > "$NAV"
n=0
for f in "$RUN"/losses/*.bc21; do
  ...
  nice -n 10 "$REPO/tools/replay-dump.sh" "$f" --navstats 2>/dev/null | grep "^  nav " | \
    ...
    awk -v opp="$opp" -v map="$map" -v side="$side" -v bot="$BOTNAME" '
      { if (NR==1) { a=$0 } else { b=$0 } }
      END { split(a,x," "); split(b,y," ");
            if (x[1] ~ /^(bot|g_iter)/) { split(a,u," "); split(b,t," ") } else { split(b,u," "); split(a,t," ") }
            printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", opp, map, side, u[5], t[5], u[2], t[2], u[7], t[7], u[3], t[3], u[4], t[4], u[6], t[6] }' >> "$NAV"
