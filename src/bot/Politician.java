...
public strictfp class Politician extends Robot {
    ...
    private void guard() throws GameActionException {
        ...
        if (rc.isReady()) {
            int minValue = C.SPEECH_CONV_DIV > 0 ? Math.max(C.SPEECH_MIN_VALUE, conv / C.SPEECH_CONV_DIV) : C.SPEECH_MIN_VALUE;
            if (conv >= C.BIG_GUARD) minValue = Math.max(minValue, conv / C.BIG_GUARD_DIV);   // Iteration 54: a big guard is not spent on one muckraker
            int r2 = bestSpeech(minValue);
    ...
    private void capture() throws GameActionException {
        ...
        RobotInfo t = null;
        if (rc.canSenseLocation(target)) { t = rc.senseRobotAtLocation(target); if (t == null || t.type != RobotType.ENLIGHTENMENT_CENTER || t.team == us) { MapState.removeNeutral(target); MapState.removeEnemy(target); if (t != null && t.team == us) { MapState.sightOwnEC(target, round); if (C.ABORT_REPORT == 1) setFlag(Comms.encode(Comms.OWN_EC, MapState.stamp(round), target)); /* stamped now: seen */ } role = Roles.GUARD; Debug.log("@capture abort by=" + (t == null ? "vanished" : t.type != RobotType.ENLIGHTENMENT_CENTER ? "notEC" : "ours") + " r=" + round + " age=" + (round - birth)); return; } targetInf = t.influence; }
