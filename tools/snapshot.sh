...
mkdir -p "$DEST"
for f in src/bot/*.java; do sed -e "s/^package bot;/package $NAME;/" -e "s/^import bot\./import $NAME./" -e "s/^import static bot\./import static $NAME./" -e "s/ARCHETYPE = 0;/ARCHETYPE = $ARCH;/" -e "s/DEBUG = true;/DEBUG = false;/" "$f" > "$DEST/$(basename "$f")"; done
