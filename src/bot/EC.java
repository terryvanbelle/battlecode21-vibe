...
public strictfp class EC extends Robot {
    ...
    private void build(int inf, boolean danger) throws GameActionException {
        ...
        else if (MapState.nEnemy > 0 && enemyEcInf > 0 && inf - floored() >= Math.max(200, enemyEcInf / 2) && capturers < 3) { captureTargetIdx = -1; t = RobotType.POLITICIAN; cost = Math.min(inf - floored(), enemyEcInf + 40); role = Roles.CAPTURE; }
        else if (!danger && slanderers < C.MAX_SLANDERERS && Econ.bestSize(inf - reserve()) >= 21 && (guards >= slanderers / 3)) { t = RobotType.SLANDERER; cost = Econ.bestSize(Math.min(inf - reserve(), C.MAX_SLANDERER_SIZE)); role = Roles.ECON; }   // source, not sink: never floored
        else if (inf - floored() >= 20 && (guards < C.GUARD_BASE + slanderers / 2 || (danger && guards < C.MAX_GUARDS))) { t = RobotType.POLITICIAN; cost = Math.min(Math.max(20, (inf - floored()) / 4), 60); role = Roles.GUARD; }   // floored too (dose 1 left it unfloored: guards ate the surplus, slanderers starved)
        ...
        else if (!danger && slanderers < C.MAX_SLANDERERS && Econ.bestSize(inf - reserve()) >= 21) { t = RobotType.SLANDERER; cost = Econ.bestSize(Math.min(inf - reserve(), C.MAX_SLANDERER_SIZE)); role = Roles.ECON; }   // source, not sink: never floored
        else if (inf - floored() >= 100 && guards < C.MAX_GUARDS) { t = RobotType.POLITICIAN; cost = Math.min(inf - floored(), Math.max(50, inf / 3)); role = Roles.GUARD; }
        else if (MapState.nEnemy > 0 && inf - floored() >= 300 && capturers < 3) { captureTargetIdx = -1; t = RobotType.POLITICIAN; cost = inf - floored(); role = Roles.CAPTURE; }   // rich and idle: throw everything at the enemy EC
        else if (inf - floored() >= C.SPARE_MIN) {
    ...
    /** reserve() plus the Iteration 17 floor: what non-capture spending must leave in the EC. */
    private int floored() {
        int f = Math.min(C.FLOOR_MAX, round * C.FLOOR_PER_ROUND / 100);
        if (round >= C.FLOOR_RELEASE_TO) f = 0; else if (round > C.FLOOR_RELEASE_FROM) f = f * (C.FLOOR_RELEASE_TO - round) / (C.FLOOR_RELEASE_TO - C.FLOOR_RELEASE_FROM);
        return Math.max(reserve(), f);
    }
