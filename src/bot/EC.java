...
public strictfp class EC extends Robot {
    ...
    private void build(int inf, boolean danger) throws GameActionException {
        ...
        if (C.ARCHETYPE == 3) {   // politician rush: 2 scouts, 2 small slanderers, then every 100+ influence becomes a capture politician aimed at the enemy EC
        ...
        else if (inf >= 20 && (guards < C.GUARD_BASE + slanderers * C.GUARD_PER_SLANDERER_NUM / C.GUARD_PER_SLANDERER_DEN / 2 || (danger && guards < C.MAX_GUARDS))) {   // Iteration 7: the same ratio here (was slanderers/2) t = RobotType.POLITICIAN; cost = Math.min(Math.max(20, inf / 4), 60); role = Roles.GUARD; }
            ...
            if (guards < slanderers * C.GUARD_PER_SLANDERER_NUM / C.GUARD_PER_SLANDERER_DEN + 2 || spare >= 300) { t = RobotType.POLITICIAN; cost = Math.min(spare, Math.max(20, spare / 3)); role = Roles.GUARD; }   // a big bank buys big guards whatever the ratio
