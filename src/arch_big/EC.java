...
public strictfp class EC extends Robot {
    ...
    private void build(int inf, boolean danger, boolean econDanger) throws GameActionException {
        ...
        if (C.ARCHETYPE == 4) {   // arch_big: the doctrine measured from rzhan11 (TRAINING_LOG, 2026-09-19)
            ...
            Direction d4 = spawnDir(t, cost, role); if (d4 == null) return;
        ...
        if (C.ARCHETYPE == 3) {   // politician rush: 2 scouts, 2 small slanderers, then every 100+ influence becomes a capture politician aimed at the enemy EC
            ...
            if (C.INVEST_MIN > 0 && !econDanger && spare >= C.INVEST_MIN && slanderers < C.SPEND_SLANDERER_CAP) { t = RobotType.SLANDERER; cost = Econ.bestSize(Math.min(spare, C.MAX_SLANDERER_SIZE)); role = Roles.ECON; Debug.log("@invest size=" + cost + " spare=" + spare); }   // Iteration 29: the bank compounds instead of standing guard
            else if (guards < slanderers + 2 || spare >= 300) { t = RobotType.POLITICIAN; cost = Math.min(spare, Math.max(20, spare / 3)); role = Roles.GUARD; }
