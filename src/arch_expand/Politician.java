...
public strictfp class Politician extends Robot {
    ...
    private void capture() throws GameActionException {
        ...
        RobotInfo t = null;
        if (rc.canSenseLocation(target)) { t = rc.senseRobotAtLocation(target); if (t == null || t.type != RobotType.ENLIGHTENMENT_CENTER || t.team == us) { MapState.removeNeutral(target); MapState.removeEnemy(target); if (t != null && t.team == us) MapState.addOwnEC(target); role = Roles.GUARD;
            // who got there first? "ours" means we sent two capturers to the same centre and wasted one;
            // "enemy" means we lost the race. The old log said only "gone/ours" and could not tell them apart.
            Debug.log("@capture abort by=" + (t == null ? "vanished" : t.type != RobotType.ENLIGHTENMENT_CENTER ? "notEC" : "ours") + " r=" + round + " age=" + (round - birth)); return; } targetInf = t.influence; }
